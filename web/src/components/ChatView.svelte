<script lang="ts">
  ...
  async function loadExistingSession(sid: string): Promise<void> {
    try {
      ...
      // Fetch engine events from the backend and replay them into chat messages
      const events = await api.getSessionEvents(sid);
      messages = events.flatMap(eventToMessages);
      ...
      // Only connect WS if session is still active
      const isActive = selectedSession?.active !== false;
      if (isActive) {
        ...
        // Don't set isConnected=true prematurely — wait for onopen via the WS callbacks
        isConnected = false;
      } else {
        ws = null;
        isConnected = false;
  ...
  /** Convert a single EngineEvent to ChatMsg(s) — used for both live WS and history replay. */
  function eventToMessages(event: EngineEvent): ChatMsg[] {
    switch (event.type) {
      case 'thinking':
        return [{
          role: 'status',
      ...
      case 'observation':
        return [{ role: 'observation', content: event.content, bytes: event.bytes, skill: event.skill }];
      case 'safety_refusal':
      ...
      case 'report':
        return [{ role: 'report', content: event.markdown, filename: event.filename ?? undefined }];
      case 'status':
        return [{ role: 'status', content: event.message }];
      case 'error':
        return [{ role: 'error', content: event.message }];
      case 'done':
        return [{ role: 'status', content: `✓ ${event.reason}` }];
      default:
        return [];
    }
  }
