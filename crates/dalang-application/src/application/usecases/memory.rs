...
pub fn compact_messages(messages: &mut Vec<dalang_domain::domain::models::Message>) {
    ...
    let tail: Vec<dalang_domain::domain::models::Message> = messages[middle_end..].to_vec();
    messages.truncate(1); // Keep system prompt
    messages.push(dalang_domain::domain::models::Message::user(&compact));
