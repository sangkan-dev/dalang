//! Storage Ports.
//!
//! Defines contracts for persisting auth credentials, chat sessions, and scan reports.
...
// ── Report files (markdown artifacts) ─────────────────────────────────────────

/// One entry in the report index (matches dashboard API shape).
...
/// Outbound port for listing and reading `dalang_report_*.md` files.
pub trait ReportStorage: Send + Sync {
    fn list_reports(&self) -> Result<Vec<ReportIndexEntry>>;
    /// Read raw markdown. Implementations must reject filenames outside `dalang_report_*.md`.
