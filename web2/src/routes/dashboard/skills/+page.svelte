	...
	<header class="flex items-center justify-between gap-3">
		...
		<div class="flex gap-2">
			<input
				...
				class="rounded-lg border border-[color:var(--color-border)] bg-transparent px-3 py-2 text-sm text-[color:var(--color-base-text)]"
			/>
			<button
						...
						>
							<div class="flex items-center justify-between gap-2">
								...
								{#if skill.enabled === false}
									<span
										class="rounded bg-[color:var(--color-rust)]/20 px-1.5 py-0.5 text-[10px] text-[color:var(--color-rust)]"
										>off</span
									>
								{/if}
							</div>
			...
			<div class="surface-panel max-h-[72vh] overflow-auto p-4">
				{#if selectedSkill}
					<div class="mb-3 flex items-start justify-between gap-3">
						<div>
							<h3 class="text-lg font-semibold text-[color:var(--color-base-text)]">
								{selectedSkill.name}
							</h3>
					...
					<div class="space-y-2 text-sm">
						<p>
							<span class="text-[color:var(--color-ash)]">Tool:</span>
							<code>{selectedSkill.tool_path ?? 'Browser-native'}</code>
						</p>
						<p>
							<span class="text-[color:var(--color-ash)]">Root required:</span>
							{selectedSkill.requires_root ? 'Yes' : 'No'}
						</p>
						{#if selectedSkill.args?.length}
							<p>
								<span class="text-[color:var(--color-ash)]">Args:</span>
